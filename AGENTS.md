...
- Accepted visual target: `/workspace/scratch/8c53b47f3924/generated_images/exec-aecf2985-9b12-4d8e-bd29-2a71ff0edd62.png`.
- The selected shell is the light editorial direction: warm white/pale straw, wine primary actions, restrained antique gold, dark editorial serif headings, clean sans-serif controls.
- Useful analytical elements from the dark metrics direction are retained only where they change a decision. Metrics must show window, sample size, missing data, uncertainty, and a drill-down; they must not crowd `Hoje`.
- The visual direction is approved and must remain stable while functionality grows. Every decorative or analytical element must earn its place by doing at least one job: capture a fact, reveal a pattern, or guide a small next action. Beauty without operational value is not a feature.
- `Hoje` is the focused entry point, not the whole product. Preserve the fixed centers `Hoje`, `Agenda`, `Registrar`, `Mentor`, and `Arquivo` and the full Master Plan scope behind them.
- Confirmed finance hubs are Mercado Pago, Banco do Brasil, and PicPay. Never infer balances or credentials from screenshots.
- Durable history is 365 days; default dashboards use 60 days. Missing records are unknown, never absence, non-adherence, or failure.
- Legacy Diario de Obstetricia and Cefaleia/Bruxismo files remain untouched and may only be imported through a reversible migration.
- Obstetric maternity calculators live as an internal `Internato` subarea, never as a sixth global center or a persisted domain. Preserve the compact Apple-inspired calculator selector and the seven explicit instruments: DUM, USG, DUM × USG comparison, ART/milestones, quantified blood loss, maternal shock index, and Apgar. Their fields and results are ephemeral React state: do not collect patient identifiers, write them to IndexedDB/localStorage, emit analytics, or include them in Archive/backup. Switching calculators may preserve drafts; leaving Obstetrícia must unmount the workspace and discard them.
