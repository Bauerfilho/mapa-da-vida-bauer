...
import { useMobileRuntimeMode, useScreenPortal } from "./PhoneFrame";
...
export function BottomSheet({
  ...
  const runtimeScreenHeight =
    isNative && typeof window !== "undefined"
      ? window.innerHeight
      : device.geometry.screen.height;
  const sheetHeight = Math.round(runtimeScreenHeight * snap);
  const effectiveHeight = Math.max(260, sheetHeight - Math.min(keyboardHeight, 180));
  const sheetBottom = isNative
    ? keyboardHeight
    : device.platform === "android"
      ? Math.max(device.geometry.safeArea.bottom, keyboardHeight)
      : keyboardHeight;
  const sheetMaxHeight = isNative
    ? `min(${Math.round(snap * 100)}dvh, calc(100dvh - env(safe-area-inset-top, 0px) - max(${keyboardHeight}px, env(safe-area-inset-bottom, 0px))))`
    : effectiveHeight;
