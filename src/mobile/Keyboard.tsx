...
import { useMobileRuntimeMode } from "./PhoneFrame";
...
function isTextEntry(element: Element | null): element is HTMLElement {
  return (
    ...
    (element.matches("input:not([type='range']):not([type='checkbox']):not([type='radio']), textarea") ||
...
export function KeyboardProvider({ children }: PropsWithChildren) {
  ...
  const nativeViewportBaselineRef = useRef(0);
  const fullHeight = isNative ? nativeKeyboardHeight : device.geometry.keyboard.height;

  useEffect(() => {
    if (!isNative) {
      setNativeKeyboardHeight(0);
      nativeViewportBaselineRef.current = 0;
      return;
    }
    ...
    nativeViewportBaselineRef.current = visibleViewportBottom();
    ...
    const updateViewportInset = () => {
      ...
      if (!hasTextFocus) {
        nativeViewportBaselineRef.current = visibleBottom;
        setNativeKeyboardHeight(0);
        return;
      }

      if (visibleBottom > nativeViewportBaselineRef.current - 40) {
        nativeViewportBaselineRef.current = Math.max(
          nativeViewportBaselineRef.current,
          visibleBottom,
        );
      }
      ...
      setNativeKeyboardHeight(occludedHeight >= 80 ? Math.round(occludedHeight) : 0);
    };
    ...
    visualViewport?.addEventListener("resize", updateViewportInset, { passive: true });
    visualViewport?.addEventListener("scroll", updateViewportInset, { passive: true });
  ...
  const value = useMemo<KeyboardContextValue>(
    ...
    [dragOffset, focusedElement, fullHeight, isDragging, isNative, visible],
  );
