...
test("desktop defaults to the calibrated phone preview", async ({ page }) => {
  await expect(page.getByTestId("phone-stage")).toHaveAttribute("data-runtime-mode", "preview");
  await expect(page.getByTestId("device-picker")).toBeVisible();
  await expect(page.locator(".phone-bezel")).toHaveCount(1);
  await expect(page.locator(".status-bar")).toHaveCount(1);
  await expect(page.getByTestId("home-indicator")).toHaveCount(1);
  await expect(page.getByTestId("keyboard-dock")).toHaveCount(1);
});

test("native override renders one edge-to-edge app surface without simulated chrome", async ({ page }) => {
  await page.goto("/tests/runtime-fixture.html?native=1");
  ...
  const viewportFit = await page.evaluate(() => {
    ...
    return {
      ...
      runtimeMode: document.documentElement.dataset.mobileRuntime,
      touchAction: getComputedStyle(
        document.querySelector<HTMLElement>('[data-testid="mobile-scroll"]')!,
      ).touchAction,
    };
  });
...
test("native mode uses the browser input and keeps sheets inside the app portal", async ({ page }) => {
  await page.goto("/tests/runtime-fixture.html?fixture=keyboard&native=1");
  const input = page.getByLabel("Message");

  await input.click();
  await expect(input).toBeFocused();
  await expect(page.getByTestId("mobile-app-viewport")).toHaveAttribute(
  ...
  expect(
    await page.getByTestId("bottom-sheet").evaluate((sheet) =>
      Boolean(sheet.closest("[data-phone-screen]")),
    ),
  ).toBe(true);
});

test("small coarse screens select native mode and preview=1 remains a QA escape hatch", async ({
  browser,
}, testInfo) => {
  ...
  await expect(mobilePage.getByTestId("phone-stage")).toHaveAttribute(
    "data-runtime-mode",
    "native",
  );

  await mobilePage.goto("/tests/runtime-fixture.html?preview=1");
  await expect(mobilePage.getByTestId("phone-stage")).toHaveAttribute(
    "data-runtime-mode",
    "preview",
  );
  await expect(mobilePage.getByTestId("device-picker")).toBeVisible();

  await context.close();
});

test("standalone display selects native mode unless preview is explicitly forced", async ({ page }) => {
  ...
  await page.goto("/tests/runtime-fixture.html?preview=1");
  await expect(page.getByTestId("phone-stage")).toHaveAttribute("data-runtime-mode", "preview");
});
...
test("keyboard and its attached footer dismiss on the same transition", async ({ page }) => {
  ...
  ).toEqual({ visibility: "hidden", opacity: "0", pointerEvents: "none" });
